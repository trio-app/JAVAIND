<?php echo $header; ?>
<br>
<br>
<br>
<div class="container">
        <div class="row">
          
            <div class="regular-content">
              <header>
                  <h3><span class="text--semi-bold">PERENCANAAN DALAM ISO 9001:2015</span></h3>
              </header>
              <p>Dalam ISO 9001 : 2015 yang mengedepankan Manajemen Berbasis Risiko (Risk Based-Thinking), 
                  <span style="color: #4286f4"><b>Perencanaan</b></span>adalah salah satu klausul yang termasuk di dalam ISO 9001 : 2015, dan di dalam klausul inilah pemikiran berbasis risiko itu akan di bahas, sebelum kita melanjutkan pembahasan ini kita terlebih dahulu memahami arti kata perencanaa sebagai berikut :
              </p>
              <p>
                  <span style="color: #4286f4"><b>Perencanaan</b></span> secara garis besar diartikan seagai proses mendefinisikan tujuan organisasi, membuat strategi untuk mencapai tujuan itu, dan mengembangkan rencana aktivitas kerja organisasi. Pada dasarnya yang dimaksud 
                 <span style="color: #4286f4"><b>perencanaan</b></span> yaitu memberi jawaban atas pertanyaan-pertanyaan apa (what), siapa (who), kapan (when), dimana (where), mengapa (why), dan bagaimana (how). Jadi perencanaan yaitu fungsi manajemen puncak yang berhubungan dengan pemilihan dari sekumpulan 
                 kegiatan-kegiatan dan pemutusan tujuan-tujuan, kebijakan - kebijakan serta program - program yang dilakukan. Perencanaan merupakan proses terpenting dari semua fungsi manajemen karena tanpa perencanaan apa yang dilakukan akan berjalan tidak maksimal.
                 <br>
                 Di dalam ISO 9001 : 2015 sudah ditentukan pada klausul <span style="color: #4286f4"><b> 6.</b></span> tentang <span style="color: #4286f4"><b>Perencanaan</b></span> :
              </p>
              <br>
              <p><span style="color: #4286f4"><b>6.1 Tindakan Untuk Menangani Resiko dan Peluang. </b></span>
                  <br>
                  Klausul ini adalah penjabaran lebih rinci tentang penerapan pemikiran berbasis risiko (risk-based thinking).
                  <br>
                  <span style="color: #4286f4" class="text--light"><b>6.2. Sasaran Mutu dan Perencanaan Untuk Mencapainya.</b></span>
                  <br>
                  Organisasi harus menetapkan sasaran mutu yang relevan sesuai bagian atau fungsi yang sesuai dengan tujuan organisasi.
                  <br>
                  <span style="color: #4286f4" class="text--light"><b>6.3.  Perencanaan Perubahan</b></span>
                  <br>
                  Pada klausul ini ISO 9001:2015 mensyaratkan bahwa bila dianggap perlu untuk mengubah sitem manajemen mutu, maka perubahan harus dilaksanakan secara terencana dan sistematik.
              </p>
              <p>
                  Perencanaan merupakan tahapan paling penting dari suatu fungsi manajemen, terutama dalam menghadapi lingkungan eksternal yang berubah dinamis. Dalam era globalisasi ini, perencanaan harus lebih mengandalkan prosedur yang rasional dan sistematis.
              </p>
              <p>
                  Suatu perencanaan juga terdapat berbagai hambatan dalam penetapan tujuan. Hambatan tersebut antara lain tujuan yang tidak tepat, sistem penghargaan yang tidak tepat, penolakan terhadap perubahan dan keterbatasan.
              </p>
              <p>
                  Dalam ISO 9001:2015 klausul 6. Tentang perencanaan, organisasi harus mampu mengalisis semua aspek yang dapat mempengaruhi kinerja organisasi, karena ini sangat mempengaruhi terhadap hasil kinerja yang akan di dapatkan oleh organisasi.
              </p>
              <p>Jadi, mau organisasi atau perusahaan anda lebih baik lagi ? mari ber ISO, kami siap memberikan konsultasi, Hubungi kami di ……………..</p>
              <br>
              <p>Salam Sukses & Semangat ber ISO</p>
            </div>
         
        </div>
      </div>
<?php echo $footer; ?>

